...
public class Account {
	...
	public ArrayList getTransactions() throws Exception{
		try{
			...
			//int i; // extra line
			for(int i = 0; i < dbTransactionList.size(); i++){ // null pointer exception
			...
			throw new Exception("Can't retrieve transactions from the database"); // missing ex instance
		}
	}

	public Transaction makeTransactionFromDbRow(DbRow row){
		double currencyAmountInPounds = Double.parseDouble(row.getValueForField("amt")); // final variable
	...
	// Override the equals method //annotation
	public boolean equals(Account o) {
		return o.getAccountNumber() == getAccountNumber(); // check account numbers are the same (Equals)
	}
}*/